...
* для заполнения контента в БЭМ-блоке лучше создать папку ```props``` с файлами pug, в которых будут прописаны mock-объекты для чистоты pug-файла

Пример сложной структуры папки с БЭМ-блоком и использование ```props``` смотреть в ```src/blocks/modules/example``` (страница main.pug)
